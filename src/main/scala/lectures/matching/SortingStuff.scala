...
object SortingStuff extends App {
  ...
  private def putStuffInRightBox(item: Stuff, stuffBox: StuffBox) = item match {
    case it@(item: Watches) if it.cost > 1000 => stuffBox.copy(watches = it :: stuffBox.watches)
    case it@(item: Boots) if it.brand == "Adidas" || it.brand == "Converse" => stuffBox.copy(boots = it :: stuffBox.boots)
    case it@(item: Book) if it.isInteresting => stuffBox.copy(books = it :: stuffBox.books)
